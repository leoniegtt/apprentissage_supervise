...
ada_model = AdaBoostClassifier( learning_rate= 1, n_estimators= 50)

X_train = scaler.fit_transform(X_train)
ada_model.fit(X_train, y_train["PINCP"])
...
data_test = pd.concat([pd.DataFrame(X_test, columns=features.columns), pd.DataFrame(ada)], axis=1)
corr_test = data_test.corr(numeric_only=False)
plt.figure(figsize=(12, 10))
...
result = permutation_importance(ada_model, X_test, y_test["PINCP"], n_repeats=20, random_state=0)
